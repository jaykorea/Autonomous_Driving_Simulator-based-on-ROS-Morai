...
class ForwardObjectDetector(object):
    ...
    def detect_object(self, vehicle_state):
        ...
        tmp_t = np.array([[np.cos(vehicle_state.velocity.y), -np.sin(vehicle_state.velocity.y), tmp_translation.x],
                          [np.sin(vehicle_state.velocity.y), np.cos(vehicle_state.velocity.y), tmp_translation.y],
                          [0, 0, 1]])
