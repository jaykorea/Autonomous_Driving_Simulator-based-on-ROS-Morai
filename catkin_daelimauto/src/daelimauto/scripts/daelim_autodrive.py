...
class wecar_planner():
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            ...
            if ((self.distance_flag2 == True) and (self.mission2_flag == True or self.mission3_flag == True) and (self.person_flag == False)):
                self.distance_flag2 = False
                self.distance_flag3 = False
            ...
            if ((self.distance_flag3 == True) and (self.mission2_flag == True or self.mission3_flag == True) and (self.person_flag == True)):
                self.distance_flag2 = False
                self.distance_flag3 = False
                self.person_flag = False
    ...
    def objectDataCB(self,data):
        ...
        if len(bbox) != 0:
            for i in range(len(bbox)):
                if ( bbox[i].Class == 'person' and bbox[i].probability >= self.m_pub_threshold ) or (bbox[i].Class == 'bicycle' and bbox[i].probability >= self.m_pub_threshold ):
                    objbox = bbox[i]
    ...
    def lidarfovdataCB(self,data):
        for i in range(90):
            ...
            if 0.0 < data.ranges[i] <= 0.45:
                self.distance_flag3 = True
