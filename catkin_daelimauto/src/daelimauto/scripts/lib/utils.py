...
def findLocalPath(ref_path,status_msg):
    ...
    #currnet_z=status_msg.position.z
    current_waypoint=0
    min_dis=float('inf') ## 여기수정 z값 추가
...
class purePursuit :
    def __init__(self):
        ...
        self.min_lfd=1
        self.max_lfd=20
        self.vehicle_length=0.5  # 30, 0.3
        self.steering=0
...
def latticePlanner(ref_path,global_vaild_object,vehicle_status,current_lane):
    ...
    look_distance=int(vehicle_status[3]*3.6*0.2*2) #default = 2
    # print("\nlook_dist :\n", look_distance)
    if look_distance < 5 :
        look_distance=3     # default min 5m  5 = 5, 8 = 8
    if look_distance > 8 :
        look_distance=5   
    if len(ref_path.poses)>look_distance :
        ...
        #lane_off_set = [(i - 5 // 2) * 0.3 for i in range(5)]
        ...
        for end_point in local_lattice_points :
            ...
            a[0]=ps
            a[1]=0
            a[2]=3.0*(pf-ps)/(xf*xf) #defautl = 3.0
            a[3]=-2.0*(pf-ps)/(xf*xf*xf) #default = -2.0
        ...
        add_point_size=int(vehicle_status[3]*4*3.6) # default 3,4,3.6
        ...
        lane_weight = [abs((i - 6 // 2) * 10) for i in range(5)] 
